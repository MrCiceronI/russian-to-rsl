...
# Теперь используем абсолютный путь к файлу
verb_aspects_path = os.path.join(CURRENT_DIR, "verb_aspects.json")
with open(verb_aspects_path, "r", encoding="utf-8") as file_verb_aspects:
...
def map_to_lemma(word: dict, gloss_dict: dict) -> str:
    ...
    if upos_dict[word["upos"]] == gloss_dict[best_match[0]]["часть_речи"] or (
        word["upos"] == "NOUN"
        and gloss_dict[best_match[0]]["часть_речи"] == "существительное"
    ):
